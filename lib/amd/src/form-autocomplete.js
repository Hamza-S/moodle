...
define(['jquery', 'core/log', 'core/str', 'core/templates', 'core/notification'], function($, log, str, templates, notification) {
    ...
    var uniqueId = $.now();
    // Unique value - not an integer so it doesn't conflict with possible DB ids.
    var showMoreValue = 'showmore';
     ...
     * @param {JQuery} originalSelect The JQuery object matching the hidden select list.
     * @param {Number} activationIndex The numerical index of the item to activate.
     */
    var updateSuggestions = function(options, state, query, originalSelect, activationIndex) {
        // Find the elements in the DOM.
        var inputElement = $(document.getElementById(state.inputId));
        var suggestionsElement = $(document.getElementById(state.suggestionsId));
        ...
        if (state.moreAvailable === true) {
            suggestions[suggestions.length] = {label: options.viewMoreString, value: showMoreValue};
        }
        ...
        templates.render(
            ...
            if (originalSelect.attr('data-notice')) {
                ...
                // creating a new tag.
                if (!options.tags) {
                    // By default we select the first item in the list.
                    // If the "showmore" option was selected, we scroll to the start of the
                    // newly fetched elements.
                    if (typeof activationIndex == 'undefined') {
                        activationIndex = 0;
                    }
                    activateItem(activationIndex, state);
                }
            } else {
    ...
    var selectCurrentItem = function(options, state, originalSelect) {
        ...
        if (selectedItemValue === showMoreValue) {
            require([options.ajax], function(ajaxHandler) {
                ...
                inputElement.focus();
                ajaxHandler.showMore();
                element = suggestionsElement.children('[aria-selected=true]');
                // Find it's index.
                current = suggestionsElement.children('[aria-hidden=false]').index(element);
                updateAjax(e, options, state, originalSelect, ajaxHandler, current);
            });
        } else if (options.closeSuggestionsOnSelect) {
     ...
     * @method updateAjax
     * @private
     * @param {Event} e The event that triggered this update (optional).
     * @param {Object} options The original options for the autocomplete.
     ...
     * @param {Number} activationIndex The numerical index of the item to activate.
     */
    var updateAjax = function(e, options, state, originalSelect, ajaxHandler, activationIndex) {
        ...
        // Note the transport function can resolve with an optional second paramenter which
        // indicates there may be more results available on the server. This will show an extra option
        // in the select list with 'showmore' as the value.
        // The ajax handler must implement a 'showMore' function for this to work.
        ajaxHandler.transport(options.selector, query, function(results, moreAvailable) {
            ...
            // Remember if we limited the results that were fetched.
            if (typeof moreAvailable != 'undefined') {
                // Ensure we have a boolean value.
                state.moreAvailable = (moreAvailable === true);
            }
            ...
            updateSuggestions(options, state, '', originalSelect, activationIndex);
    ...
    return /** @alias module:core/form-autocomplete */ {
         ...
         * @param {String} viewMoreString - Text to display when there is a link to view more results.
         * @return {Promise}
         */
        enhance: function(selector, tags, ajax, placeholder, caseSensitive, showSuggestions, noSelectionString,
            ...
            if (typeof viewMoreString === "undefined") {
                str.get_string('viewmore', 'core').done(function(result) {
                    options.viewMoreString = result;
                }).fail(notification.exception);
            }
