...
define(['jquery', 'core/log', 'core/str', 'core/templates', 'core/notification'], function($, log, str, templates, notification) {
    ...
    var addNavigation = function(options, state, originalSelect) {
        ...
        var suggestionsElement = $(document.getElementById(state.suggestionsId));
        suggestionsElement.parent().on('mousedown', '#' + state.suggestionsId, function(e) {
            var element = $(e.target).closest('[role=option]');
            // Prevent events triggered by clicking on the scrollbar. This
            // stops the blur event from being fired on the input field which
            // would close the suggestions list instead of scrolling.
            if (element.length == 0) {
                e.preventDefault();
            }
        });
