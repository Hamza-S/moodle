...
Feature: Teacher can search and enrol users one by one into the course
  ...
  Scenario: Searching for a non-existing user
    ...
    And I open the autocomplete suggestions for the field "Select users"
  ...
  @javascript
  Scenario: If there are more than 100 matching users, allow pagination of the results
    Given the following "users" exist:
      | username    | firstname | lastname | email                   |
    ...
    And I press "Enrol users"
    And I open the autocomplete suggestions for the field "Select users"
    And I click on "View more" item in the autocomplete list
    And I click on "Student 103" item in the autocomplete list
    Then I should see "Student 103"
    And I click on "Enrol users" "button" in the "Enrol users" "dialogue"
    Then I should see "Active" in the "Student 103" "table_row"
